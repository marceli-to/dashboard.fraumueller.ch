...
class ImportOrders extends Command
{
    ...
    private function importCreditCardFile(string $filename): int
    {
        ...
        $this->withProgressBar(0, function () use ($handle, $headers, &$imported, &$skipped) {
    ...
    private function importTwintFile(string $filename): int
    {
        ...
        $this->withProgressBar(0, function () use ($lines, $dataStartIndex, &$imported, &$skipped, &$orderCounter) {
            for ($i = $dataStartIndex; $i < count($lines); $i++) {
                ...
                try {
                    ...
                    $orderId = 'TW' . str_pad($orderCounter, 4, '0', STR_PAD_LEFT);
